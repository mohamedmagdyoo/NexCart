...
struct NexCartApp: App {
    ...
    var productDetailsViewModel=ProductDetailViewModel(addCartUseCase: AddCartUseCase(productDetailsRepo: ProductDetailRepoImpl(productDetailsService: ProductDetailsService(networkClient: ApiService()))))
    var dummy: Product {
         Product(
             id: 1,
    ...
    var body: some Scene {
        WindowGroup {
            ZStack{
                ProductDetailView(product: dummy, productViewModel: productDetailsViewModel)
