...
final class CompleteOrderUseCase: CompleteOrderUseCaseProtocol {
    ...
    func execute(
        ...
        let lineItems: [OrderLineItemBody] = allItems.compactMap { item in
            ...
            return OrderLineItemBody(variantId: variantId, quantity: item.quantity, price: String(format: "%.2f", item.price))
        }
        ...
        print(orderBody.lineItems.count)
        print(orderBody.lineItems.first?.price)
